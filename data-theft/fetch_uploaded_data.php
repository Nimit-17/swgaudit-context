...
try {
    ...
    for ($chunkNumber = 1; $chunkNumber <= $totalDataChunks; $chunkNumber++) {
        if (!isset($chunks[$chunkNumber])) {
            throw new Exception("Incomplete transmission: missing data chunk " . $chunkNumber);
        }
    ...
    if (isset($metadata["encodedLength"]) && strlen($encodedData) !== (int)$metadata["encodedLength"]) {
        throw new Exception("Incomplete transmission: reconstructed encoded data length does not match original");
    }

    $fileData = base32_decode_dns($encodedData);
    if (isset($metadata["size"]) && strlen($fileData) !== (int)$metadata["size"]) {
        throw new Exception("Incomplete transmission: reconstructed file size does not match original");
    }
